CREATE DATABASE `db_authority_system`;
...
CREATE TABLE `sys_user` (
  ...
  `account_non_expired` tinyint NOT NULL COMMENT '帐户是否过期(1-未过期，0-已过期)',
  `account_non_locked` tinyint NOT NULL COMMENT '帐户是否被锁定(1-未过期，0-已过期)',
  `credentials_non_expired` tinyint NOT NULL COMMENT '密码是否过期(1-未过期，0-已过期)',
  `enabled` tinyint NOT NULL COMMENT '帐户是否可用(1-可用，0-禁用)',
